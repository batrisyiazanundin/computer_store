...
class SuppliersTable extends Table
{
    ...
    public function validationDefault(Validator $validator): Validator
    {
        $validator
            ->scalar('supplier_id')
            ->maxLength('supplier_id', 100)
            ->allowEmptyString('supplier_id', null, 'create');

        $validator
            ->scalar('supp_name')
            ->maxLength('supp_name', 100)
            ->allowEmptyString('supp_name');

        $validator
            ->scalar('supp_add')
            ->maxLength('supp_add', 100)
            ->allowEmptyString('supp_add');

        $validator
            ->scalar('supp_pnum')
            ->maxLength('supp_pnum', 100)
            ->allowEmptyString('supp_pnum');
